...
async def record_and_trim(max_duration_sec: int = 30, sample_rate: int = 16000,
                          vad_threshold: float = 0.5, vad_mode: int = 3) -> dict:
    """Record from the microphone, VAD-trim silence, store a 16-bit mono WAV
    under ~/ingest/audio/, and return the same dict the phone.capture.audio
    tool returns. Extracted to module level so the Phase 6 voice session can
    reuse the capture path without going through the MCP tool closure."""
...
def register(mcp):
    ...
    async def capture_audio(max_duration_sec: int = 30, sample_rate: int = 16000,
        ...
        return await record_and_trim(max_duration_sec, sample_rate,
                                     vad_threshold, vad_mode)
